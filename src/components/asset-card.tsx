...
export function AssetCard({ asset, quote, ticks, active, fresh, onFocus }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onFocus}
      className={`bay flex h-full w-full flex-col p-3.5 text-left ${active ? "bay-live" : ""}`}
    >
      ...
      {asset.kind === "dex" && windows ? (
        ...
        <div className="mt-3 flex gap-4 border-t border-rule pt-2 font-mono text-[10px] uppercase tracking-wider text-ghost">
          ...
          {quote && (
            <span className="ml-auto normal-case tracking-normal">{quote.source}</span>
          )}
        </div>
      )}
    </button>
